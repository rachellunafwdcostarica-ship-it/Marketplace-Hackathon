...
--      SOLA VEZ (permanente). Para juniors, crea también la
--      fila mínima en public.estudiantes.
--
-- Nota sobre admin:
--   Los usuarios admin se crean manualmente vía service_role
--   (bypasea RLS). No pasan por el flujo OAuth → onboarding.
--   Por eso assign_my_role rechaza 'admin' y 'moderador'.
--
...
--
--    - Solo acepta 'junior' o 'empresario' (nunca admin).
--    - Actualiza id_rol SOLO si todavía es NULL (permanencia).
--    - Para 'junior': crea también la fila mínima en estudiantes.
...
declare
  ...
  v_updated  integer;
begin
  -- Solo roles de usuario final (jamás admin ni moderador)
  if p_role not in ('junior', 'empresario') then
  ...
  -- Para junior: crear la fila mínima en estudiantes
  -- Los demás campos tienen defaults o son nullable.
  if p_role = 'junior' then
