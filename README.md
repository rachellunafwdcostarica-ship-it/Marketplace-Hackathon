...
- **Sistema de Calificación de Empresarios (Evaluaciones).** Permite que los egresados califiquen a los empresarios tras una contratación. La base de datos calcula automáticamente la reputación promedio de cada empresario mediante el trigger `trg_reputacion_empresario`. Además, para flexibilizar la retroalimentación sin obligar al cierre inmediato de la contratación en el sistema, la política RLS se configuró para permitir el envío de valoraciones en periodos de contrato tanto `vigente` como `finalizado`.
