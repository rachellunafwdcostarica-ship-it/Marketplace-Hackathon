...
export async function verificarEgresado(userId: string): Promise<Result<void>> {
  ...
  // Casting a 'usuarios' (que sí existe y tiene 'correo') temporalmente
  // para engañar al tipado porque egresados_fwd_oficial no está en
  // los tipos autogenerados de Supabase.
  const { data: fwdRecord, error: fwdError } = await adminClient
    .from('egresados_fwd_oficial' as 'usuarios')
    .select('correo')
